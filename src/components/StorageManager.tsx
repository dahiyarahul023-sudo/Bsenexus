      ...
      <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 space-y-4">
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 pt-2 border-t border-slate-200/80 dark:border-slate-800">
          ...
          <ActionButton
            ...
            isLoading={isPruning}
            loadingText={`Pruning ${prunePercentage}%...`}
            variant="danger"
            size="md"
            disabled={hasPinConfigured && !adminPin.trim()}
            icon={<Trash2 className="w-3.5 h-3.5" />}
          >
            Execute {prunePercentage}% Safe Prune
          </ActionButton>
        </div>
      ...
      <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 space-y-3">
        ...
        <div className="flex flex-wrap gap-2.5 pt-1">
          <ActionButton
            ...
            isLoading={testingAlertLevel === 50}
            loadingText="Dispatching..."
            variant="secondary"
            size="sm"
            disabled={testingAlertLevel !== null && testingAlertLevel !== 50}
            icon={<Send className="w-3 h-3 text-blue-500" />}
            className="bg-blue-50 hover:bg-blue-100 dark:bg-blue-950/50 dark:hover:bg-blue-900/60 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800/80"
          >
            Test 50% Notice Alert
          </ActionButton>

          <ActionButton
            onClick={() => handleTestAlert(90)}
            isLoading={testingAlertLevel === 90}
            loadingText="Dispatching..."
            variant="secondary"
            size="sm"
            disabled={testingAlertLevel !== null && testingAlertLevel !== 90}
            icon={<AlertTriangle className="w-3 h-3 text-amber-500" />}
            className="bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/50 dark:hover:bg-amber-900/60 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800/80"
          >
            Test 90% Warning Alert
          </ActionButton>

          <ActionButton
            onClick={() => handleTestAlert(100)}
            isLoading={testingAlertLevel === 100}
            loadingText="Dispatching..."
            variant="danger"
            size="sm"
            disabled={testingAlertLevel !== null && testingAlertLevel !== 100}
            icon={<AlertOctagon className="w-3 h-3" />}
          >
            Test 100% Critical Alert
          </ActionButton>
        </div>
      </div>
    </div>
